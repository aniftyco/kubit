declare module '@ioc:Kubit/Lucid/Orm' {
  ...
  import { ProfilerContract, ProfilerRowContract } from '@ioc:Kubit/Profiler';
   ...
   */
  export type OptionalTypedDecorator<PropType> = <TKey extends string, TTarget extends { [K in TKey]?: PropType }>(
    target: TTarget,
    property: TKey
  ) => void;
   ...
   */
  export type ScopeFn = <Model extends LucidModel, Scope extends QueryScopeCallback = QueryScopeCallback<Model>>(
    callback: Scope
  ) => QueryScope<Scope>;
  ...
  export interface ModelQueryBuilderContract<Model extends LucidModel, Result = InstanceType<Model>>
    ...
    paginate(
      page: number,
      perPage?: number
    ): Promise<Result extends LucidRow ? ModelPaginatorContract<Result> : SimplePaginatorContract<Result>>;
