...
import { ManagesFrequenciesContract, Time } from '@ioc:Kubit/Scheduler';
...
export default class ManagesFrequencies implements ManagesFrequenciesContract {
  ...
  public lastDayOfMonth(time: Time = '00:00') {
    return this.spliceIntoPosition(2, DateTime.now().setZone(this.currentTimezone).endOf('month').day).dailyAt(time);
  }
  ...
  public yearly() {
    return this.spliceIntoPosition(0, 0).spliceIntoPosition(1, 0).spliceIntoPosition(2, 1).spliceIntoPosition(3, 1);
  }
