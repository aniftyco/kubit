...
export class AssetsManager implements AssetsManagerContract {
  ...
  public entryPointScriptTags(name: string): string {
    ...
    return scripts.map((url) => `<script src="${url}"${stringifyAttributes(mergedAttributes)}></script>`).join('\n');
  }
  ...
  public entryPointStyleTags(name: string): string {
    ...
    return links
      .map((url) => `<link rel="stylesheet" href="${url}"${stringifyAttributes(styleAttributes)} />`)
      .join('\n');
  }
