...
import { ExtendCallback, SlugifyConfig, SlugifyManagerContract, StrategiesList } from '@ioc:Kubit/ORM';
...
export class SlugifyManager implements SlugifyManagerContract {
  ...
  private makeExtendedStrategy(strategy: string, config: SlugifyConfig) {
    if (!this.extendedStrategies.has(strategy)) {
      throw new Exception(`"${strategy}" is not a valid slugify strategy`, 500, 'E_INVALID_SLUGIFY_STRATEGY');
    }
