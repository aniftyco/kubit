...
import { LucidModel, LucidRow, SlugifyConfig, SlugifyStrategyContract } from '@ioc:Kubit/ORM';
...
export class DbIncrementStrategy extends SimpleStrategy implements SlugifyStrategyContract {
   ...
   */
  private async getSlugForSqlite(model: LucidModel, field: string, columnName: string, slug: string) {
   ...
   */
  private async getSlugForOldMysql(model: LucidModel, field: string, columnName: string, slug: string) {
    const rows = await model
      .query()
      .select(field)
      .where(field, slug)
