...
export class PivotHelpers {
   ...
   */
  public wherePivot(varition: 'or' | 'and' | 'not' | 'orNot', key: any, operator?: any, value?: any) {
  ...
  public whereInPivot(varition: 'or' | 'and' | 'not' | 'orNot', key: any, value?: any) {
    ...
    key = Array.isArray(key) ? key.map((one) => this.prefixPivotTable(one)) : this.prefixPivotTable(key);
