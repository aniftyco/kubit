...
import { DBQueryCallback, DialectContract, QueryClientContract, TransactionClientContract } from '@ioc:Kubit/Database';
import {
  LucidModel,
  LucidRow,
  ModelAdapterOptions,
...
export class ModelQueryBuilder extends Chainable implements ModelQueryBuilderContract<LucidModel> {
  ...
  private async execQuery() {
    ...
    const rows = await new QueryRunner(this.client as any, this.debugQueries, queryData).run(this.knexQuery);
  ...
  public returning(columns: any): this {
    if (this.client.dialect.supportsReturningStatement) {
      columns = Array.isArray(columns) ? columns.map((column) => this.resolveKey(column)) : this.resolveKey(columns);
   ...
   */
  public andWhereDoesntHave(relationName: any, callback: any, operator?: string, value?: any): this {
    return this.addWhereHas(relationName, 'not', operator, value, callback);
  }
  ...
  public async paginate(page: number, perPage: number = 20): Promise<any> {
    ...
    const countQuery = this.clone().clearOrder().clearLimit().clearOffset().clearSelect().count('* as total').pojo();
