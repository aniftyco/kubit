...
import { collectValues, ensureRelation, isObject, managedTransaction, normalizeCherryPickObject } from '../utils';
...
export class BaseModel implements LucidRow {
   ...
   */
  private static newUpWithOptions(payload: any, options?: ModelAdapterOptions, allowExtraProperties?: boolean) {
    const row = new this();
  ...
  public static $addColumn(name: string, options: Partial<ColumnOptions>) {
    ...
    const column: ModelColumnOptions = {
      ...
      serializeAs:
        options.serializeAs !== undefined ? options.serializeAs : this.namingStrategy.serializedName(this, name),
   ...
   */
  protected static $addHasOne(name: string, relatedModel: () => LucidModel, options: RelationOptions<ModelRelations>) {
   ...
   */
  protected static $addHasMany(name: string, relatedModel: () => LucidModel, options: RelationOptions<ModelRelations>) {
  ...
  public static async findMany(value: any[], options?: ModelAdapterOptions) {
    ...
    return this.query(options).whereIn(this.primaryKey, value).orderBy(this.primaryKey, 'desc').exec();
  }
   ...
   */
  public static async firstOrNew(searchPayload: any, savePayload?: any, options?: ModelAssignOptions): Promise<any> {
    /**
   ...
   */
  public static async firstOrCreate(searchPayload: any, savePayload?: any, options?: ModelAssignOptions): Promise<any> {
    /**
     * Search using the search payload and fetch the first row
     */
    const query = this.query(options).where(searchPayload);
   ...
   */
  public static async fetchOrNewUpMany(uniqueKeys: any, payload: any, options?: ModelAssignOptions): Promise<any[]> {
   ...
   * hooks.
   */
  public static async fetchOrCreateMany(uniqueKeys: any, payload: any, options?: ModelAssignOptions): Promise<any[]> {
    uniqueKeys = Array.isArray(uniqueKeys) ? uniqueKeys : [uniqueKeys];
    const uniquenessPair = uniqueKeys.map((uniqueKey: string) => {
      return {
        key: uniqueKey,
   ...
   * of model hooks.
   */
  public static async updateOrCreateMany(uniqueKeys: any, payload: any, options?: ModelAssignOptions): Promise<any> {
    uniqueKeys = Array.isArray(uniqueKeys) ? uniqueKeys : [uniqueKeys];
    const uniquenessPair = uniqueKeys.map((uniqueKey: string) => {
      return {
        key: uniqueKey,
  ...
  protected prepareForAdapter(attributes: ModelObject) {
    ...
    return Object.keys(attributes).reduce((result, key) => {
      ...
      const value = typeof column.prepare === 'function' ? column.prepare(attributes[key], key, this) : attributes[key];
   ...
   */
  private shouldSerializeField(serializeAs: string | null, fields?: CherryPickFields): serializeAs is string {
    /**
  ...
  public async load(relationName: any, callback?: any) {
    ...
    await preloader.sideload(this.$sideloaded).processAllForOne(this, Model.$adapter.modelClient(this));
  }
