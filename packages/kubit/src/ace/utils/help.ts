 ...
 */
export function printHelp(commands: SerializedCommand[], flags: CommandFlag[], aliases: Aliases): void {
  ...
  sortAndGroupCommands(commands).forEach(({ group, commands: groupCommands }) => {
    ...
    groupCommands.forEach(({ commandName, description }) => {
      ...
      console.log(`  ${logger.colors.green(displayName)} ${whiteSpace}  ${logger.colors.dim(descriptionRows)}`);
    });
  });
...
export function printHelpFor(command: SerializedCommand, aliases: Aliases): void {
  ...
  console.log('');
  console.log(
    `${logger.colors.yellow('Usage:')} ${command.commandName} ${logger.colors.dim(command.args.map(wrapArg).join(' '))}`
  );
  ...
  if (commandAliases.length) {
    console.log('');
    console.log(`${logger.colors.yellow('Aliases:')} ${logger.colors.green(commandAliases.join(', '))}`);
  }
  ...
  if (args.length) {
    ...
    args.forEach(({ displayName, description = '', width }) => {
      ...
      console.log(`  ${logger.colors.green(displayName)} ${whiteSpace}  ${logger.colors.dim(descriptionRow)}`);
    });
  }
