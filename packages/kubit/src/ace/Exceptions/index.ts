...
export class MissingArgumentException extends Exception {
   ...
   */
  public static invoke(name: string, command: CommandConstructorContract): MissingArgumentException {
...
export class InvalidFlagException extends Exception {
   ...
   */
  public static invoke(prop: string, expected: string, command?: CommandConstructorContract): InvalidFlagException {
  ...
  public handle(error: InvalidFlagException) {
    logger.error(logger.colors.red(`Expected "--${error.flagName}" to be a valid "${error.expectedType}"`));
  }
}
