...
export class MakeMigration extends BaseCommand {
  ...
  public async run(): Promise<void> {
     ...
     * Template stub
     */
    const stub = join(__dirname, '../../../..', 'templates', this.table ? 'migration-alter.txt' : 'migration-make.txt');
      ...
      .apply({
        toClassName() {
          return function (filename: string, render: (text: string) => string) {
            const migrationClassName = string.camelCase(tableName || render(filename).replace(prefix, ''));
