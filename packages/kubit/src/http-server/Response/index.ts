...
export class Response extends Macroable implements ResponseContract {
   ...
   * ```
   */
  public stream(body: ResponseStream, errorCallback?: (error: NodeJS.ErrnoException) => [string, number?]): void {
   ...
   */
  public abortUnless(condition: any, body: any, status?: number): void {
    if (!condition) {
      this.abort(body, status);
    }
  }
