...
export class Store {
  ...
  public add(route: RouteJSON): this {
    ...
    lodash.merge(routeJSON, lodash.pick(route, ['pattern', 'handler', 'meta', 'middleware', 'name']));
  ...
  public match(
    ...
    return {
      route: route,
      routeKey: matchingDomain ? `${matchingDomain}-${method}-${route.pattern}` : `${method}-${route.pattern}`,
