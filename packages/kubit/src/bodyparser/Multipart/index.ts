...
import { MultipartContract, MultipartStream, PartHandler as PartHandlerType } from '@ioc:Kubit/BodyParser';
...
export class Multipart implements MultipartContract {
  ...
  private processConfig(config?: Parameters<MultipartContract['process']>[0]) {
     ...
     */
    this.maxFieldsSize =
      typeof this.config!.fieldsLimit === 'string' ? bytes(this.config.fieldsLimit) : this.config!.fieldsLimit;
   ...
   * ```
   */
  public onFile(name: string, options: Parameters<MultipartContract['onFile']>[1], handler: PartHandlerType): this {
      ...
      this.form.on('error', (error: Error) => {
        ...
        process.nextTick(() => {
          ...
          if (error.message.match(/stream ended unexpectedly/)) {
            ...
            reject(new Exception('Fields size in bytes exceeded', 413, 'E_REQUEST_ENTITY_TOO_LARGE'));
          } else {
            reject(error);
          }
        });
