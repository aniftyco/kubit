...
import { DatabaseContract, FileNode, QueryClientContract, TransactionClientContract } from '@ioc:Kubit/Database';
import { MigratedFileNode, MigrationListNode, MigratorContract, MigratorOptions } from '@ioc:Kubit/Database/Migrator';
...
export class Migrator extends EventEmitter implements MigratorContract {
   ...
   */
  private async recordMigrated(client: QueryClientContract, name: string, executionResponse: boolean | string[]) {
    if (this.dryRun) {
   ...
   * the migration
   */
  private async recordRollback(client: QueryClientContract, name: string, executionResponse: boolean | string[]) {
    if (this.dryRun) {
      this.migratedFiles[name].queries = executionResponse as string[];
      return;
    }
  ...
  private async getMigratedFiles() {
    const rows = await this.client.query<{ name: string }>().from(this.schemaTableName).select('name');
  ...
  public on(event: 'upgrade:version', callback: (payload: { from: number; to: number }) => void): this;
