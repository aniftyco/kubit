...
export class I18n extends Formatter implements I18nContract {
   ...
   */
  private formatValidatorMessage(identifier: string, data: Record<string, string>, forceNotify = false): string | null {
  ...
  public validatorMessages(messagesPrefix: string = 'validator.shared'): {
    ...
    return {
      '*': (field, rule, arrayExpressionPointer, options) => {
         ...
         */
        const fieldRuleMessage = this.formatValidatorMessage(`${messagesPrefix}.${field}.${rule}`, data);
        if (fieldRuleMessage) {
   ...
   */
  public formatMessage(identifier: string, data?: Record<string, any>, fallbackMessage?: string): string {
    this.lazyLoadTranslations();
    const message = this.getMessage(identifier);
