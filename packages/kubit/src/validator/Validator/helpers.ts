...
import { ErrorReporterConstructorContract, NodeSubType, NodeType, ParsedRule, SchemaRef } from '@ioc:Kubit/Validator';
...
export function resolveAbsoluteName(field: string, otherField: string): string {
  return field[0] === '/' ? field.slice(1) : otherField.split('.').slice(0, -1).concat(field).join('.');
}
...
export function wrapCompile<T extends any>(
  ...
  customCallback?: (options: any[], type: NodeType, subtype: NodeSubType, rulesTree: any) => Partial<ParsedRule<T>>
) {
  return function (type: NodeType, subtype: NodeSubType, options: T, rulesTree: any): ParsedRule<T> {
    /**
