import { DialectContract, QueryClientContract } from '@ioc:Kubit/Lucid/Database';
...
export class OracleDialect implements DialectContract {
  ...
  public async truncate(table: string, cascade: boolean = false) {
    return cascade ? this.client.rawQuery(`TRUNCATE ${table} CASCADE;`) : this.client.rawQuery(`TRUNCATE ${table};`);
  }
