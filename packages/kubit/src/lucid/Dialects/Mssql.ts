...
import { DialectContract, MssqlConfig, QueryClientContract } from '@ioc:Kubit/Lucid/Database';
...
export class MssqlDialect implements DialectContract {
  ...
  public async dropAllTables() {
    ...
    const ignoredTables = (this.config.wipe?.ignoreTables || []).map((table) => `"${table}"`).join(', ');
