...
import { LucidModel, LucidRow, ModelAdapterOptions, ModelObject } from '@ioc:Kubit/Lucid/Orm';
...
export class FactoryBuilder implements FactoryBuilderContract<FactoryModelContract<LucidModel>> {
  ...
  private async makeRelations(modelInstance: LucidRow, ctx: FactoryContextContract) {
    for (let { name, count, callback } of this.withBelongsToRelations) {
      ...
      await relation.useCtx(ctx).merge(this.recursiveAttributes).make(modelInstance, callback, count);
    }
   ...
   */
  private async createRelations(modelInstance: LucidRow, ctx: FactoryContextContract, cycle: 'before' | 'after') {
    ...
    for (let { name, count, callback } of relationships) {
      const relation = this.factory.getRelation(name);
      await relation.useCtx(ctx).merge(this.recursiveAttributes).create(modelInstance, callback, count);
    }
  }
   ...
   * to the database
   */
  public tap(callback: (row: LucidRow, state: FactoryContextContract, builder: this) => void): this {
