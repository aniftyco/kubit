...
import { HasManyClientContract, LucidModel, LucidRow, ModelAssignOptions, ModelObject } from '@ioc:Kubit/Lucid/Orm';
...
export class HasManyQueryClient implements HasManyClientContract<HasMany, LucidModel> {
   ...
   * Create instance of the related model
   */
  public async createMany(values: ModelObject[], options?: ModelAssignOptions): Promise<LucidRow[]> {
    const parent = this.parent;
   ...
   */
  public async firstOrCreate(search: any, savePayload?: any, options?: ModelAssignOptions): Promise<LucidRow> {
    return managedTransaction(this.parent.$trx || this.client, async (trx) => {
      this.parent.$trx = trx;
      await this.parent.save();
      ...
      return this.relation.relatedModel().firstOrCreate(valuesToPersist, savePayload, { client: trx, ...options });
    });
  }
  ...
  public async updateOrCreate(
    ...
    return managedTransaction(this.parent.$trx || this.client, async (trx) => {
      ...
      return this.relation.relatedModel().updateOrCreate(valuesToPersist, updatePayload, { client: trx, ...options });
    });
  }
