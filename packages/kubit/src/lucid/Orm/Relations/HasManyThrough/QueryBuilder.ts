...
import { HasManyThroughQueryBuilderContract, LucidModel, LucidRow } from '@ioc:Kubit/Lucid/Orm';
  ...
  implements HasManyThroughQueryBuilderContract<LucidModel, LucidModel>
{
  ...
  protected applyConstraints() {
    ...
    if (['delete', 'update'].includes(this.queryAction())) {
      this.whereIn(this.prefixRelatedTable(this.relation.throughForeignKeyColumnName), (subQuery: this) => {
