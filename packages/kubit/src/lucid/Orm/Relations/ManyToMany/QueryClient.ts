...
import { OneOrMany, QueryClientContract, TransactionClientContract } from '@ioc:Kubit/Lucid/Database';
import { LucidModel, LucidRow, ManyToManyClientContract, ModelAssignOptions, ModelObject } from '@ioc:Kubit/Lucid/Orm';
...
export class ManyToManyQueryClient implements ManyToManyClientContract<ManyToMany, LucidModel> {
  ...
  public async saveMany(
    ...
    await managedTransaction(this.parent.$trx || this.client, async (trx) => {
      ...
      const relatedForeignKeyValues = related.reduce<Record<string, ModelObject>>((result, one, index) => {
