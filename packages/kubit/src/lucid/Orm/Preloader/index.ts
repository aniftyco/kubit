...
export class Preloader implements PreloaderContract<LucidRow> {
  ...
  private async processRelation(name: string, parent: LucidRow, client: QueryClientContract) {
    ...
    const query = relation.eagerQuery(parent, client).debug(this.debugQueries).sideload(this.sideloaded);
   ...
   * during eagerloading
   */
  private async processRelationForMany(name: string, parent: LucidRow[], client: QueryClientContract) {
    const { relation, callback } = this.preloads[name];
    const query = relation.eagerQuery(parent, client).debug(this.debugQueries).sideload(this.sideloaded);
