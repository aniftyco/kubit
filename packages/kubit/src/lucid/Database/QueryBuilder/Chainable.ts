...
import { ChainableContract, DBQueryCallback } from '@ioc:Kubit/Lucid/Database';
...
export abstract class Chainable extends Macroable implements ChainableContract {
  ...
  private normalizeAggregateColumns(columns: any, alias?: any): any {
    if (columns.constructor === Object) {
      return Object.keys(columns).reduce((result, key) => {
        ...
        result[key] = typeof value === 'string' ? this.resolveKey(value) : this.transformValue(value);
        return result;
      }, {});
    }
  ...
  public whereIn(columns: any, value: any): this {
    value = Array.isArray(value) ? value.map((one) => this.transformValue(one)) : this.transformValue(value);
  ...
  public whereRaw(sql: any, bindings?: any): this {
    ...
    if (bindings) {
      bindings = Array.isArray(bindings) ? bindings.map((binding) => this.transformValue(binding)) : bindings;
  ...
  public orderBy(column: any, direction?: any): this {
    ...
    if (Array.isArray(column)) {
      const transformedColumns = column.map((col) => {
        ...
        if (col.column) {
          col.column = typeof col.column === 'string' ? this.resolveKey(col.column) : this.transformValue(col.column);
          return col;
        }
  ...
  public union(queries: any, wrap?: boolean): this {
    ...
    queries = Array.isArray(queries) ? queries.map((one) => this.transformValue(one)) : this.transformValue(queries);
   ...
   */
  public if(condition: any, matchCallback: (query: this) => any, noMatchCallback?: (query: this) => any): this {
   ...
   */
  public unless(condition: any, matchCallback: (query: this) => any, noMatchCallback?: (query: this) => any): this {
    let matched: any = condition;
    if (typeof condition === 'function') {
      matched = condition();
    }
   ...
   */
  public match(...blocks: ([condition: any, callback: (query: this) => any] | ((query: this) => any))[]): this {
