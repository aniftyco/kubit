...
import { FileNode, QueryClientContract, TransactionClientContract } from '@ioc:Kubit/Lucid/Database';
import { CherryPickFields, LucidRow, ModelObject, RelationshipsContract } from '@ioc:Kubit/Lucid/Orm';
 ...
 */
export function ensureRelation<T extends RelationshipsContract>(name: string, relation?: T): relation is T {
  if (!relation) {
...
export function ensureRelationIsBooted(relation: RelationshipsContract) {
  if (!relation.booted) {
    throw new Exception('Relationship is not booted. Make sure to call boot first', 500, 'E_RUNTIME_EXCEPTION');
  }
}
