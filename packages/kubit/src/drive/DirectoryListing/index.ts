  ...
  implements DirectoryListingContract<Driver, T>
{
  ...
  constructor(
    ...
    private listing: (this: Driver) => AsyncGenerator<T>
  ) {}
   ...
   */
  public recursive(
    next: (current: T, depth: number, driver: Driver) => Promise<string | null> | string | null = (current) =>
    ...
    return this.pipe(async function* (source) {
      for await (const item of source) {
        ...
        if (location === null) {
          ...
          const nextWithDepth =
            next.length > 1 ? (current: T, depth: number, driver: Driver) => next(current, depth + 1, driver) : next;

          // list the returned location from next function and also apply the chain to it
          const list = chain.reduce((listing, fn) => listing.pipe(fn), this.list!(location)).recursive(nextWithDepth);
