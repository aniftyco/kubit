...
export class FakeEmitter implements FakeEmitterContract {
   ...
   */
  public exists(eventOrCallback: string | ((event: { name: string; data: any }) => boolean)): boolean {
   ...
   * Get selected events
   */
  public filter(eventOrCallback: string | ((event: { name: string; data: any }) => boolean)): any[] {
