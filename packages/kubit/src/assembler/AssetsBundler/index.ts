...
export class AssetsBundler extends Emittery {
  ...
  private notifyAboutEncore() {
    ...
    this.logger.info(`building frontend assets. Use { ${this.logger.colors.dim().yellow('--no-assets')} } to disable`);
  }
  ...
  public async startDevServer(): Promise<DevServerResponse> {
    ...
    const childProcess = execa(this.binaryName, ['dev-server'].concat(this.encoreArgs), this.execaOptions);
