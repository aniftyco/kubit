...
import { cleanup, fs, getConfig, resetTables, setup, setupApplication } from '../../test-helpers'
...
test.group('Query client', (group) => {
  ...
  test('get query client in read only mode', async ({ assert }) => {
    ...
    const client = new QueryClient('read', connection, app.container.use('Kubit/Event'))
  ...
  test('get query client in write only mode', async ({ assert }) => {
    ...
    const client = new QueryClient('write', connection, app.container.use('Kubit/Event'))
  ...
  test('get query client dialect version', async ({ assert }) => {
    ...
    const client1 = new QueryClient('write', connection, app.container.use('Kubit/Event'))
