...
test.group('Transaction | query', (group) => {
  ...
  test('commit insert', async ({ assert }) => {
    ...
    const results = await new QueryClient('dual', connection, app.container.use('Kubit/Event'))
      .query()
      .from('users')
    assert.isArray(results)
  ...
  test('commit insert inside a self managed transaction', async ({ assert }) => {
    ...
    await new QueryClient('dual', connection, app.container.use('Kubit/Event')).transaction(
      async (db) => {
        await db.insertQuery().table('users').insert({ username: 'virk' })
      }
    )
