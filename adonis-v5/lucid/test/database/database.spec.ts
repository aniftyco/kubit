...
import { cleanup, fs, getConfig, getDb, setup, setupApplication } from '../../test-helpers'
...
test.group('Database', (group) => {
  ...
  test('register all connections with the manager', ({ assert }) => {
    ...
    const db = new Database(config, app.logger, app.profiler, app.container.use('Kubit/Event'))
  ...
  test('pass profiler to query client', async ({ assert }) => {
    ...
    const db = new Database(config, app.logger, profiler, app.container.use('Kubit/Event'))
