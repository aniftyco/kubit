...
import { cleanup, fs, getConfig, mapToObj, setup, setupApplication } from '../../test-helpers'
...
test.group('ConnectionManager', (group) => {
  ...
  test('do not connect until connect is called', async ({ assert }) => {
    const manager = new ConnectionManager(app.logger, app.container.use('Kubit/Event'))
