...
import { MigratorContract, MigratorOptions } from '@ioc:Kubit/Lucid/Migrator'
import { AdapterContract, LucidModel, LucidRow } from '@ioc:Kubit/Lucid/Orm'
...
export function getDb(application: ApplicationContract) {
  ...
  return new Database(
    config,
    application.container.use('Kubit/Logger'),
    application.container.use('Kubit/Profiler'),
    application.container.use('Kubit/Event')
  ) as DatabaseContract
}
...
export async function setupApplication(
  ...
  if (process.env.DEBUG) {
    app.container.use('Kubit/Event').on('db:query', (query) => {
      console.log({
        model: query.model,
        sql: query.sql,
